...
describe('Security101', async function () {
    ...
    beforeEach(async () => {
        ...
        const VictimToken = await ethers.getContractFactory('contracts/contracts_optimized/OptimizedSecurity101.sol:Security101');
        victimToken = await VictimToken.deploy();
        await victimToken.deployed();
        await helpers.setBalance(victimToken.address, ethers.utils.parseEther('10000'));
    });
    ...
    describe('Gas target (redacted)', function () {
        it('Hack the contract', async function () {
            ...
            const txn = await attackerContract.connect(attacker).deploy(victimToken.address, {
    ...
    afterEach('hack must succeed', async function () {
        ...
        expect(await ethers.provider.getBalance(victimToken.address)).to.be.equal(0, 'victim contract not drained');
        expect(await ethers.provider.getTransactionCount(attacker.address)).to.equal(1, 'only one transaction allowed');
    });
});
