...
describe('Distribute', async function () {
    ...
    beforeEach(async () => {
        ...
        instance = await ContractFactory.deploy([acct1.address, acct2.address, acct3.address, acct4.address], {
    ...
    describe('Business logic', function () {
        it('The functions MUST perform as expected', async function () {
            await expect(instance.distribute()).to.be.rejectedWith('cannot distribute yet');
            ...
            expect(await ethers.provider.getBalance(acct1.address)).to.equal(new BigNumber.from(ethers.utils.parseEther('0.25')));
            expect(await ethers.provider.getBalance(acct2.address)).to.equal(new BigNumber.from(ethers.utils.parseEther('0.25')));
            expect(await ethers.provider.getBalance(acct3.address)).to.equal(new BigNumber.from(ethers.utils.parseEther('0.25')));
            expect(await ethers.provider.getBalance(acct4.address)).to.equal(new BigNumber.from(ethers.utils.parseEther('0.25')));
        });
    });
});
